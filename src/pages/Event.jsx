...
import { validEvent } from "../data/valid";
...
function Events() {
    ...
    const handleSubmit = (event) => {
        ...
        if (validEvent(values).valid) {
            try {
                // set of image in storage
                setUploading(true)
                // save in db
                axios.post(`${API_URL}/upload-event`, values)
                    .then(res => {
                        console.log(res)
                    })
                    .catch(err => {
            ...
            setErrors(validEvent(values).message)
        }
    };
        ...
        <div className=" flex justify-center items-center">
            <form onSubmit={handleSubmit} className=" flex justify-around items-center gap-5 flex-col my-20">
                <div className=" font-cairo flex flex-col justify-center gap-4 items-center">
                    <h2 className=" text-2xl font-bold ">رفع حدث جديد</h2>
                ...
                <div className="flex justify-around items-center gap-5 flex-col p-5 mx-auto">
                    <div className="flex flex-col font-cairo w-full justify-center gap-4 mx-auto">
                        <label className="text-end" htmlFor="">عنوان الحدث</label>
                    ...
                    <div className="flex flex-col font-cairo w-full justify-center gap-4 mx-auto">
                        <label className="text-end" htmlFor="">تاريخ الحدث</label>
                        <input name="date" onChange={handleChange} className=" md:w-[500px] mx-6 md:mx-0 h-[40px] outline-none rounded-md shadow-md w-[300px] " type="date" />
                    </div>
                    <div className="flex flex-col font-cairo w-full justify-center gap-4 mx-auto">
                        <label className="text-end" htmlFor="">تفاصيل الحدث</label>
